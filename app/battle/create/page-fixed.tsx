...
function CreateBattleContent() {
  ...
  async function createRoom(code: string) {
    ...
    if (newBattle) {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'battles', filter: `id=eq.${newBattle.id}` }, (payload) => {
          if (payload.new.status === 'ready' && payload.new.player2_id) {
            setWaiting(false)
            setTimeout(() => { router.push(`/battle/${newBattle.id}`) }, 1500)
          }
        })
        .subscribe()
    }
  ...
  function handleCancel() {
    if (battle) { supabase.from('battles').delete().eq('id', battle.id) }
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex items-center justify-center p-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative max-w-md w-full">
        <button onClick={handleCancel} className="inline-flex items-center gap-2 text-dark-400 hover:text-white mb-6">
        ...
        <div className="card text-center">
          ...
          <div className="bg-dark-700 rounded-xl p-4 mb-4">
            <div className="text-4xl font-mono font-bold tracking-widest text-ice-400 mb-2">{roomCode}</div>
            <button onClick={copyCode} className="inline-flex items-center gap-2 text-sm text-dark-400 hover:text-white transition-colors">
              {copied ? (<><Check className="w-4 h-4 text-green-400" /><span className="text-green-400">Copied!</span></>) : (<><Copy className="w-4 h-4" /><span>Copy Code</span></>)}
            </button>
          </div>
          <div className="py-8">
            {waiting ? (
              <><div className="flex items-center justify-center gap-2 mb-4">
                <img src={getAvatarUrl(user?.username || '', user?.avatar_url)} alt="You" className="w-12 h-12 rounded-full border-2 border-fire-500" />
                ...
                <div className="w-12 h-12 rounded-full border-2 border-dashed border-dark-600 flex items-center justify-center"><span className="text-dark-500">?</span></div>
              </div><p className="text-dark-400">Waiting for opponent to join...</p></>
            ) : (
              <><div className="text-green-400 font-bold mb-2">Opponent Found!</div><Loader2 className="w-6 h-6 animate-spin mx-auto" /><p className="text-dark-400 mt-2">Starting battle...</p></>
            )}
          </div>
          {isDemo && (<button onClick={handleDemoStart} className="btn-fire w-full">Start Demo Battle</button>)}
        </div>
      </motion.div>
    </div>
  )
...
export default function CreateBattlePage() {
  return (<Suspense fallback={<LoadingFallback />}><CreateBattleContent /></Suspense>)
}
